...
const bodySchema = z.object({ note: z.string().min(1, "A reason is required") });
...
export async function PATCH(
  ...
  if (exchange.status !== "PENDING" && exchange.status !== "PROCESSING") {
    return NextResponse.json(
      { error: "Only pending or processing requests can be failed" },
      { status: 400 }
    );
  }
  ...
  const [, , updated] = await prisma.$transaction([
    prisma.exchangeTransaction.update({
      ...
      data: {
        ...
        processingNote: parsed.data.note,
      },
    }),
